...
状态栏为全局单条（`laststatus=3`），左侧依次是模式、coc-git 分支与本文件 `+N ~N -N` 改动、coc 诊断图标计数，中间是文件图标与路径（修改过加 `●`），右侧是 coc 服务状态、文件类型图标、`行:列` 与百分比。标签栏同样带文件图标与修改标记。光标停在某个词上时，同名词会以淡色底标出（mini.cursorword）。

## 界面

- 图标依赖 Nerd Font（终端已配置 Hack Nerd Font）。
- 命令行与消息使用 Neovim 0.12 内置的 ui2：平时底部不占行（`cmdheight=0`），按 `:` 才出现；消息在右下角小窗浮现 3 秒后消失；长输出不再弹 "Press ENTER"，`g<` 随时回看上一条，`:messages` 查看历史。
- 行号列右侧一格放 coc 的 git 竖条（绿增 / 黄改 / 红删）与诊断图标，出错行的行号同时变色；诊断内容只在光标所在行以虚拟文本显示。
- 正常模式只高亮当前行号，插入模式高亮整行；非当前窗口整体变暗。
- 补全菜单类型图标在前，选中项以幽灵文本预览在光标后。
